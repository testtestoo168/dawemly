      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Row(children: [
              Text('الإشعارات', style: GoogleFonts.tajawal(fontSize: 24, fontWeight: FontWeight.w800, color: W.text)),
              if (unread > 0) ...[SizedBox(width: 10), Container(padding: EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: W.red, borderRadius: BorderRadius.circular(6)), child: Text('$unread غير مقروء', style: GoogleFonts.tajawal(fontSize: 12, fontWeight: FontWeight.w700, color: Colors.white)))],
            ]),
          ]),
          ...
          if (_loading)
            ...
            Container(padding: EdgeInsets.all(50), decoration: BoxDecoration(color: W.white, borderRadius: BorderRadius.circular(6), border: Border.all(color: W.border)),
              child: Center(child: Column(children: [
                Icon(Icons.notifications_off, size: 40, color: W.hint),
            ...
            ...filtered.map((n) {
              ...
              return Container(
                ...
                child: Opacity(opacity: isRead ? 0.65 : 1, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 22, vertical: 16), child: Row(children: [
                  Row(children: [
                    if (!isRead && id.isNotEmpty) ...[SizedBox(width: 4), InkWell(onTap: () => _markRead(id), child: Container(width: 24, height: 24, decoration: BoxDecoration(color: Color(0xFFECFDF3), borderRadius: BorderRadius.circular(6)), child: Icon(Icons.check, size: 10, color: W.green)))],
                  ...
                  Expanded(flex: 3, child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                    Row(mainAxisAlignment: MainAxisAlignment.end, children: [
                      ...
                      const SizedBox(width: 6),
                      Flexible(child: Text(n['title'] ?? '', style: GoogleFonts.tajawal(fontSize: 14, fontWeight: isRead ? FontWeight.w500 : FontWeight.w700, color: W.text), overflow: TextOverflow.ellipsis)),
                    ]),
                    const SizedBox(height: 2),
                    Text(n['body'] ?? '', style: GoogleFonts.tajawal(fontSize: 12, color: W.sub), textAlign: TextAlign.right),
                  ])),
                  const SizedBox(width: 14),
                  Container(width: 40, height: 40, decoration: BoxDecoration(color: color.withOpacity(0.08), borderRadius: BorderRadius.circular(6)), child: Icon(_typeIcon[n['type']] ?? Icons.info, size: 18, color: color)),
                ]))),
              );
            }),
        ]),
