...
class _AdminAuditState extends State<AdminAudit> {
  ...
  Future<void> _loadLogs() async {
    try {
      final result = await ApiService.get('admin.php?action=get_audit_log&limit=200');
      final logs = result['logs'] ?? result['data'] ?? [];
      setState(() {
        _allLogs = List<Map<String, dynamic>>.from(logs);
        _loading = false;
      });
    } catch (_) {
